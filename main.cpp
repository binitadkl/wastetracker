...
int main() {
    ...
    std::string alertMessage = ""; // Declare the alertMessage variable outside of the switch
    std::string wasteLevel = "";  // Declare wasteLevel outside the switch to avoid uninitialized warning
    ...
    while (true) {
        ...
        switch (choice) {
            ...
            case 5:  // Display all bins
                ht.display();
                break;
            case 6:  // Simulate overflow alert for full bins
                std::cout << "Enter bin ID to check for overflow: ";
                std::getline(std::cin, key);

                // Get the waste level for the bin
